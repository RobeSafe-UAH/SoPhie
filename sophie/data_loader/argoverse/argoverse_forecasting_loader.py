...
"""
Argoverse Motion Forecasting is a curated collection of 324,557 scenarios, each 5 seconds long, 
for training and validation. Each scenario contains the 2D, birds-eye-view centroid of each 
tracked object sampled at 10 Hz.

To create this collection, we sifted through more than 1000 hours of driving data from our fleet 
of self-driving test vehicles to find the most challenging segments — including segments that show 
vehicles at intersections, vehicles taking left or right turns, and vehicles changing lanes.

What makes this dataset stand out?

This dataset is far larger than what can currently be mined from publicly available self-driving 
datasets, and our HD maps make it easier to predict the motion of objects.

Argoverse-Forecasting contains 327790 sequences of interesting scenarios. Each sequence follows 
the trajectory of the main agent for 5 seconds, while keeping track of all other actors (e.g car, 
pedestrian). We've separated them into 208272 training sequences, 40127 validation sequences, 
and 79391 test sequences. We also provide sample data (5 sequences) in forecasting_sample.tar.gz.
...
__all__ = ["ArgoverseForecastingLoader"] # List of public objects of the module
...
class ArgoverseForecastingLoader:
    ...
    def __str__(self) -> str:
        ...
        ----------------------
        || City: {self.city}
        || # Unique Track IDs: {len(self.track_id_list)}
        || # Trajectories:     {len(self.agent_traj)}
        ----------------------"""
