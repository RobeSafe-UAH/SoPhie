...
def get_sequences_as_array(root_folder="data/datasets/argoverse/motion-forecasting/",split="train",
    ...
    Output: sequences ((A observations x B num_agents_per_obs) x 5 (features) x N (num_files)
            origin (N x 2 (x,y))
            city_ids (N x 1 (MIA/PIT))
            file_id_list (list of files of the corresponding folder)
    """
    ...
    for i,file_id in enumerate(file_id_list):
        ...
        print("Dummy filtered sequence: ", sequence.shape)
        ...
        print("Relative displacements filtered sequence: ", sequence.shape)
        ...
        curr_objs = np.unique(last_observation[:,1])
        print("Curr objs: ", curr_objs)
...
class ArgoverseMotionForecastingDataset(Dataset):
    def __init__(self, dataset_name, root_folder, obs_len=20, pred_len=30, skip=1, threshold=0.002, distance_threshold=30,
        ...
        for seq_index in range(sequences.shape[2]):
            ...
            print("1. objs_in_curr_seq: ", objs_in_curr_seq, objs_in_curr_seq.shape)
            ...
            print("2. objs_in_curr_seq: ", objs_in_curr_seq, objs_in_curr_seq.shape)
